...
import logging
from room.forms import BookingForm

logger = logging.getLogger(__name__)

# Define states
EMAIL, MENU, ROOM_SELECTION, CHECK_IN_DATE, CHECK_OUT_DATE, GUESTS, PAYMENT_METHOD, MY_BOOKINGS = range(8)
...
async def check_out_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if check_out_date <= user_data['check_in_date']:
            await update.message.reply_text(
                'Check-out date must be after the check-in date. Please enter a valid check-out date:',
                reply_markup=get_main_menu_buttons()
            )
            return CHECK_OUT_DATE
...
async def payment_method(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if payment_method == 'chapa':
        ...
        # Add Chapa payment processing logic here
...
async def my_bookings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await query.message.reply_text('An error occurred while fetching your bookings. Please try again later.')
        return ConversationHandler.END
